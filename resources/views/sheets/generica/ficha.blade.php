...
@section('sheetDelete')
<a class="btn btn-danger me-1" data-bs-toggle="modal" data-bs-target="#deleteSheetGenericaModal">Excluir</a>
@endsection
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-dark">
            ...
            <div class="modal-body text-start">
                <form id="personagemModalForm" action="{{ route('generica.updateimage', ['fichasgenerica' => $ficha->id]) }}" method="post">
                    @csrf
                    @method('put')
                    <!-- FOTO PERSONAGEM -->
...
<script>
    $('form[name="vidaForm"]').submit(function(event) {
        ...
        $.ajax({
            url: "{{ route('generica.updatelife', ['fichasgenerica' => $ficha->id]) }}",
            type: "put",
            data: $(this).serialize(),
            dataType: "json",
    ...
    $('#sheetDescription').change(function() {
        ...
        $.ajax({
            url: "{{ route('generica.updatedescription', ['fichasgenerica' => $ficha->id]) }}",
            type: "put",
            data: {
                'descricao': descricao
            },
